...
-- Run this to completely fix all accounting issues
-- =====================================================
...
-- Step 3: Create chart_of_accounts table with proper structure
...
-- Step 4: Create cash_accounts table with proper structure
...
-- Step 10: DISABLE RLS temporarily to test
...
-- Step 13: Test queries to verify everything works
SELECT 'Testing chart_of_accounts query...' as test;
SELECT id, account_code, account_name, account_type FROM chart_of_accounts LIMIT 5;

SELECT 'Testing cash_accounts query...' as test;
SELECT id, account_name, current_balance, currency FROM cash_accounts LIMIT 5;
...
-- Step 15: Enable RLS with proper policies
ALTER TABLE chart_of_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE cash_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE journal_entries ENABLE ROW LEVEL SECURITY;
ALTER TABLE journal_entry_lines ENABLE ROW LEVEL SECURITY;

-- Step 16: Create RLS policies that allow all operations
...
-- Step 17: Final test with RLS enabled
SELECT 'Final test with RLS enabled...' as test;
SELECT COUNT(*) as chart_of_accounts_count FROM chart_of_accounts;
SELECT COUNT(*) as cash_accounts_count FROM cash_accounts;

-- Step 18: Show table structure for verification
SELECT 
    table_name,
    column_name,
    data_type,
...
WHERE table_name IN ('chart_of_accounts', 'cash_accounts', 'journal_entries', 'journal_entry_lines')
