# PowerShell script to create transaction_master table
# This script will execute SQL statements directly on Supabase

$supabaseUrl = "https://oojmuyalhveuefjbwysj.supabase.co"
$supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9vam11eWFsaHZldWVmamJ3eXNqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk5MDYxOTcsImV4cCI6MjA2NTQ4MjE5N30.GqZRZJWhVkILCW0VaEiBQZ5C5_nHgGmj6vbOyk-VjrY"
...
Write-Host "Setting up transaction_master table..." -ForegroundColor Green
...
for ($i = 0; $i -lt $sqlStatements.Count; $i++) {
    ...
    Write-Host "Executing statement $($i + 1)/$($sqlStatements.Count)..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "Error executing statement $($i + 1): $($_.Exception.Message)" -ForegroundColor Red
    }
}

Write-Host "Transaction master table setup completed!" -ForegroundColor Green
